    ...
    <div class="main-content">
        ...
        <div class="main-fields">
          <div class="main-field" id="fromField" :class="{ error: v$.route.from.$error }">
            ...
            <input
              id="from"
              type="text"
              v-model="stateForm.route.from"
              :disabled="!!orderStore.lastOrderId"
              :placeholder="t('order.from_placeholder')"
              formControlName="from"
              autocomplete="off"
              @blur="v$.route.from.$touch()"
            />
            <div class="main-ac" id="fromAc" role="listbox" aria-label="Подсказки адресов"></div>
          </div>

          <label for="from" v-if="v$.route.from.$error" class="input-error">{{
            t("order.errors.from")
          }}</label>
          ...
          <div class="main-field" id="toField" :class="{ error: v$.route.to.$error }">
            ...
            <input
              id="to"
              type="text"
              v-model="stateForm.route.to"
              :disabled="!!orderStore.lastOrderId"
              :placeholder="t('order.to_placeholder')"
              formControlName="to"
              autocomplete="off"
              @blur="v$.route.to.$touch()"
            />
            <div class="main-ac" id="toAc" role="listbox" aria-label="Подсказки адресов"></div>
          </div>
          <label for="from" v-if="v$.route.to.$error" class="input-error">{{
            t("order.errors.to")
          }}</label>
        </div>
      </div>
      ...
      <div class="main-package" v-if="!orderStore.lastOrderId">
        ...
        <div class="main-sizes">
          <div
            v-for="size of DELIVERY_SIZES"
            :key="size.value"
            class="main-size-card"
            :class="{ 'is-active': stateForm.route.size === size.value }"
            @click="stateForm.route.size = size.value"
          >
            <div class="main-size-top">
              <div class="main-size-label">{{ size.value.toUpperCase() }}</div>
              <div class="main-size-rate">
                {{ size.rate }} {{ t("order.summary.price_per_km") }}
              </div>
            </div>
            <div
              class="main-size-media"
              :class="{ 'main-size-media-palleta': size.value === 'max' }"
            >
      ...
      <div class="main-speed" v-if="!orderStore.lastOrderId">
        ...
        <div class="main-speeds">
          <div
            v-for="speed of DELIVERY_SPEEDS"
            :key="speed.value"
            class="main-speed-card"
            :class="{ 'is-active': stateForm.route.speed === speed.value }"
            @click="stateForm.route.speed = speed.value"
          >
            <img
              class="main-speed-media"
              :src="`/src/assets/images/icons/${speed.value}.svg`"
              alt="Speed"
            />
      ...
      <div class="main-result" id="result" v-if="calculationResult">
        ...
        <div class="main-result-lines">
          <div class="main-result-line">
            <span>{{ t("order.summary.distance") }}</span
            ><span id="distanceValue">{{ calculationResult.distance }}</span>
          </div>
          <div class="main-result-line">
            <span>{{ t("order.summary.delivery_time") }}</span
            ><span id="durationValue">{{ calculationResult.duration }}</span>
          </div>
          <div class="main-result-line">
            <span>{{ t("order.summary.tariff") }}</span
            ><span id="rateValue">{{ calculationResult.rate }}</span>
          </div>
          <div class="main-result-line">
            <span>{{ t("order.summary.speed") }}</span
            ><span id="speedValue"></span>{{ calculationResult.speed }}
          </div>
        </div>
          ...
          <div class="main-block-info-row">
            ...
            <div class="order-fields">
              <div class="order-field">
                <input
                  :placeholder="t('order.contacts.name')"
                  autocomplete="off"
                  @blur="v$.order.name.$touch()"
                  v-model="stateForm.order.name"
                />
              </div>
              <div class="order-field">
                <input
                  :placeholder="t('order.contacts.phone')"
                  autocomplete="off"
                  inputmode="numeric"
                  appOnlyNumbers
                  @input="onPhoneInput"
                  @blur="v$.order.phone.$touch()"
                  v-model="stateForm.order.phone"
                />
              </div>
              <label v-if="v$.order.name.$error" class="input-error error-label name-error">{{
                t("order.errors.name")
              }}</label>
              <div class="error-labels">
                <label v-if="v$.order.phone.$error" class="input-error">{{
                  t("order.errors.phone")
                }}</label>
                <label v-if="v$.order.phone.minLength.$error" class="input-error">{{
                  t("order.errors.phone_digit")
                }}</label>
              </div>
            </div>
          </div>
          <div class="main-block-info-row extra-info">
            ...
            <div class="main-field textarea">
              <textarea
                ...
                v-model="stateForm.order.comment"
              ></textarea>
            </div>
          </div>
          <button
      ...
      <div v-else class="main-order-success is-visible" id="orderSuccess">
        ...
        <div class="main-success-title">
          {{ t("order.success.number") }}<span id="orderId">{{ orderStore.lastOrderId }}</span>
        </div>
...
let geoMarker: ymaps.GeoObject | null = null;
...
onMounted(() => {
  ymaps.ready(() => {
    ...
    suggestFrom.events.add(
      "select",
      (e: ymaps.IEvent) => (stateForm.route.from = e.get("item").value),
    );
...
const handleCalculate = async (): Promise<void> => {
  ...
  multiRoute.model.events.add("requestsuccess", () => {
    ...
    if (activeRoute) {
      ...
      const config = DELIVERY_SIZES.value.find(
        (s: DeliveryConfig) => s.value === stateForm.route.size,
      );
      ...
      if (config) {
        ...
        calculationResult.value = {
          ...
          total,
          speed:
            stateForm.route.speed === "fast" ? `${t("speeds.fast")}` : `${t("speeds.regular")}`,
        };
      }
...
const resetCalculation = (): void => {
  ...
  if (multiRoute) {
    ...
    mapInstance?.setCenter([41.184022, 69.164708], 5, { duration: 300 });
  }

  if (geoMarker) {
    mapInstance?.geoObjects.remove(geoMarker);
    geoMarker = null;
  }
};
</script>
